...
- **Tools → Sync Album Artwork from Discogs**: for obscure or rare releases Apple does not have
- On first launch after import, the missing artwork prompt offers Apple sync with Discogs as a follow-up option
- **Tracklist sync** uses cached Discogs release ID to skip the search step — 1 API call instead of 2
...
- Charts in a 3-row layout:
  - Row 1: Genre Breakdown donut | Collection by Decade (original release year only)
...
- Speculative decoding for ~2-3x generation speedup
- Context-aware queries with keyword-filtered album context
- **AI can be fully disabled** via Settings → AI Features checkbox
- Settings dialog is fully functional on Intel — AI-specific sections automatically hidden
...
- Discogs token; Claude API key; Last.fm key stored securely in local database
- Settings dialog scrollable on small displays (13"/12" MacBook) — Save/Cancel always visible
...
- Window size and position remembered across launches (stored in DB settings)
- All panel splitter sizes remembered across launches
